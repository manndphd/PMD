...
public class AvoidUsingStringCharAtChecker extends ConditionalChecker
{
	...
	public AvoidUsingStringCharAtChecker(CompositeParameter parameter)
	...
	{
		// Checks this method is charAt() or not

		final IMethodBinding binding = methodInvocation.resolveMethodBinding();
		if (!binding.getName().equalsIgnoreCase(CHAR_AT_METHOD_NAME))
		{
			return false;
		}

		// Checks this method is one of String's methods

		final String declaringClassName = binding.getDeclaringClass().getQualifiedName();

		if (!TypeUtil.hasRelationship(
			declaringClassName, PluginConstants.STRING_CLASS_NAME))
		{
			return false;
		}
