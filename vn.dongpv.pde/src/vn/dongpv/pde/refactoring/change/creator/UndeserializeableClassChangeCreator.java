...
import vn.dongpv.pde.rule.checker.UndeserializeableClassChecker;
import vn.dongpv.pde.util.ASTUtil;
import vn.dongpv.pde.util.TypeUtil;
import vn.dongpv.pde.util.ValidatorUtil;

public class UndeserializeableClassChangeCreator extends ChangeCreator
{

	private final ASTNode rootNode;
	private final TypeDeclaration typeDecl;

	public UndeserializeableClassChangeCreator(CompositeParameter parameter)
	...
	protected void createChange(ASTRewrite rewrite)
	{
		...
		creation.arguments().add(
			ASTUtil.createStringLiteral(
				rootNode.getAST(), "Class cannot be deserialized"));
		...
		svd.setName(
			rootNode.getAST().newSimpleName("in"));
		svd.setType(
			rootNode.getAST().newSimpleType(
				rootNode.getAST().newName(
		...
		decl.setName(
			rootNode.getAST().newSimpleName(
				UndeserializeableClassChecker.READ_OBJECT_METHOD_NAME));
		decl.setReturnType2(
